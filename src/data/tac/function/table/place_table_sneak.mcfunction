...
execute at @n[type=armor_stand,tag=table] function tac:place_table/nested_execute_0 with entity @n[type=armor_stand,tag=table] ArmorItems[0].components."minecraft:custom_data":
    #$say $(type) $(mat) $(state)
